...
class WebSocketTeleoperator(Node):
    ...
    def __init__(self):
        ...
        self.stats = {
            ...
            'last_switch_warning_time': 0,  # 上次 Switch 警告时间
        }
    ...
    async def handle_websocket_connection(self, websocket):
        ...
        self.get_logger().info(
            f'\n'
            f'🔗🔗🔗 新的 WebSocket 连接已建立 🔗🔗🔗\n'
            f'📡 客户端地址: {client_address}\n'
            f'✅ 设备连接成功，正在接收数据\n'
            f'⚠️  请确保左右手柄的 Switch 开关都推上去才能转发数据！'
        )
        ...
        if len(self.connected_clients) == 1:
            self.get_logger().info(
                f'📡 开始监听外骨骼数据\n'
                f'🎯 目标话题: {self.joint_command_topic} 和 {self.gamepad_keys_topic}\n'
                f'🔒 安全检查: 需要左右手柄 Switch 都推上去（状态为0）'
            )
        ...
        try:
            ...
            if parsed_data and parsed_data.get('valid', False):
                ...
                if self.stats['total_received'] % 2000 == 0:
                    # 检查是否有数据被 Switch 检查丢弃
                    if self.stats['dropped_by_switch_check'] > 0:
                        # 如果有大量数据被 Switch 检查丢弃，说明 Switch 没推上去
                        switch_drop_ratio = self.stats['dropped_by_switch_check'] / self.stats['total_received']
                        if switch_drop_ratio > 0.9:  # 超过90%的数据被丢弃
                            self.get_logger().warn(
                                f'\n'
                                f'⚠️⚠️⚠️ 设备已连接，但手柄开关未全部推上去！⚠️⚠️⚠️\n'
                                f'📊 统计信息:\n'
                                f'  - 已接收消息: {self.stats["total_received"]} 条\n'
                                f'  - 已转发命令: {self.stats["total_published"]} 条\n'
                                f'  - Switch检查丢弃: {self.stats["dropped_by_switch_check"]} 条 ({switch_drop_ratio*100:.1f}%)\n'
                                f'🔴 请确保左右手柄的 Switch 开关都推上去（状态为0）！\n'
                                f'💡 当前状态: 双臂控制={"启用" if self.arms_control_enabled else "禁用"}, '
    ...
    def process_and_publish_data(self, parsed_data: Dict):
        ...
        try:
            ...
            if not both_switches_up:
                ...
                # 每5秒发出一次醒目的警告提示
                current_time = time.time()
                if current_time - self.stats['last_switch_warning_time'] >= 5.0:
                    self.get_logger().warn(
                        f'\n'
                        f'🔴🔴🔴 警告：手柄开关未全部推上去！🔴🔴🔴\n'
                        f'📡 设备已连接，正在接收数据\n'
                        f'🚫 但因为 Switch 开关未推上去，数据未转发到机器人\n'
                        f'🔧 当前 Switch 状态: 左手柄={left_switch}, 右手柄={right_switch}\n'
                        f'✅ 请将左右手柄的 Switch 开关都推上去（状态应为0）\n'
                        f'📊 已丢弃 {self.stats["dropped_by_switch_check"]} 条消息'
                    )
                    self.stats['last_switch_warning_time'] = current_time
